...
class BaseModelArguments:
    ...
    add_flowhook: bool = field(
        default=False,
        metadata={"help": "Whether or not to add the flow hook to the model."},
    )
    num_layers_flow: int = field(
        default=4,
        metadata={'help': 'Set the model depth for the Flow. Default is 4.'},
    )
    hook_layers_percentile: str = field(
        default="0.5",
        metadata={'help': 'Set the hook layer percentile of hidden states for flow training.'},
    )
    flow_batch_mul: int = field(default=4, metadata={"help": "multiplier of sample for flow training."})
    flow_context_mode: str = field(default="layer_post", metadata={'help': 'The mode of context in flow. Composed by +'})
    flow_shift: str = field(default="1", metadata={"help": "Shifted tokens for flow context. Composed by + and should equal to flow_context_mode"})
    freeze_matching: bool = field(
        default=False,
        metadata={"help": "Whether or not to freeze the matching model in MLLM training."},
    )
    freeze_llm: bool = field(
        default=False,
        metadata={"help": "Whether or not to freeze the llm model in LLM training."},
    )
    ...
    def __post_init__(self):
        ...
        if self.flow_context_mode is not None:
            self.flow_context_mode = self.flow_context_mode.split("+")
        
        if self.flow_shift is not None:
            self.flow_shift = [int(flow_shift) for flow_shift in self.flow_shift.split("+")]
        
        if self.hook_layers_percentile is not None and self.hook_layers_percentile != "all":
            self.hook_layers_percentile = [float(hook_layer_percentile) for hook_layer_percentile in self.hook_layers_percentile.split("+")]
