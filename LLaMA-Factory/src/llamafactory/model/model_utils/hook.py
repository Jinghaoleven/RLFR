...
def load_hook_params(path_or_repo_id: str, model_args: "ModelArguments") -> dict[str, torch.Tensor]:
    ...
    try:
        ...
        vhead_file = cached_file(filename="model.safetensors", **kwargs)
    ...
    try:
        vhead_file = cached_file(filename="matching_model.bin", **kwargs)
        return torch.load(vhead_file, map_location="cpu")
    except Exception as err:
        err_text = str(err)
