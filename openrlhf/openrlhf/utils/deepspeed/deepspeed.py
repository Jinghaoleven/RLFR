...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer(self, model, **kwargs) -> Optimizer:
        if isinstance(model, Actor) or isinstance(model, Actor_Flow):
            model = model.model
        # Optimizer
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        is_actor = isinstance(model, Actor) or isinstance(model, Actor_Flow)
    ...
    def save_model(self, model: nn.Module, tokenizer=None, output_dir=None, **kwargs) -> None:
        if self.is_rank_0():
            os.makedirs(output_dir, exist_ok=True)

        # save model weights for ZeRO2/3
        model_to_save = self._unwrap_model(model)
        if isinstance(model_to_save, Actor_Flow):
            model_to_save = self._unwrap_model(model_to_save)
