...
from openrlhf.models.utils import compute_approx_kl, compute_reward, masked_mean, unpacking_samples, normalize_flow_score, postprocess_flow_score
from openrlhf.models.ring_attn_utils import pad_sequences, unpad_sequences
...
class Experience:
    ...
    action_flow_score: Optional[torch.Tensor] = None
    ...
    def to_device(self, device: torch.device):
        ...
        self.action_flow_score = to(self.action_flow_score, device)
    ...
    def pin_memory(self):
        ...
        self.action_flow_score = pin_memory(self.action_flow_score)
...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience_list(
        ...
        accuracy_rewards_original = torch.tensor(0)
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            action_flow_reward = postprocess_flow_score(experience.action_flow_score, args)
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        info = {
            ...
            "reward": total_reward,
            "action_score_mean": masked_mean(action_flow_score, action_mask, dim=-1),
...
class RemoteExperienceMaker(NaiveExperienceMaker):
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        # log probs
        action_log_probs, output, _, action_flow_score = self.actor(
            sequences, 
            num_actions, 
            attention_mask, 
        ...
        if args.flow_coef != 0:
            action_flow_score = normalize_flow_score(action_flow_score, action_mask, args)
        ...
        if args.flow_coef == 0:
            action_flow_score = torch.zeros_like(action_log_probs, dtype=action_log_probs.dtype, device=device)
        
        if not self.packing_samples:
            action_flow_score_mean = masked_mean(action_flow_score, action_mask, dim=-1)

        info = {
            "kl": kl_mean,
            "reward": r,
            "action_score_mean": action_flow_score_mean,
            "response_length": samples.response_length,
            "total_length": samples.total_length,
            "num_actions": num_actions,
            **specific_rewards
    ...
    def _generate_vllm(self, all_prompts: List[str], all_labels, **kwargs) -> List[Samples]:
        ...
        ban_tokens = [
            ...
            151653,        # vision_end_token_id
            151654,        # vision_token_id
            151655,        # image_token_id
            151656         # video_token_id
        ]
        bias_dict = {tid: -100 for tid in ban_tokens}
        ...
        all_prompts = sum([[prompt] * args.n_samples_per_prompt for prompt in all_prompts], [])                     #mllm_training: messages
        batch_size = (len(all_prompts) + len(llms) - 1) // len(llms)
        all_labels = sum([[label] * args.n_samples_per_prompt for label in all_labels], [])
        
        if args.llm_training:
            all_prompt_token_ids = self.tokenize_fn(all_prompts, self.prompt_max_len, padding=False)["input_ids"]   #llm_training: chat_template
        # Distribute requests to engines and collect responses to outputs
        refs = []
        # For VLM
        for i, llm in enumerate(llms):
            messages = all_prompts[i * batch_size : (i + 1) * batch_size]
            if messages and args.mllm_training:
                ...
                refs.append(
                    llm.add_mllm_requests.remote(rank, sampling_params=sampling_params, vllm_vision_input=vllm_inputs)
                )
            elif messages and args.llm_training:
                prompt_token_ids = all_prompt_token_ids[i * batch_size : (i + 1) * batch_size]
                refs.append(llm.add_llm_requests.remote(rank, sampling_params=sampling_params, prompt_token_ids=prompt_token_ids))
        ray.get(refs)
