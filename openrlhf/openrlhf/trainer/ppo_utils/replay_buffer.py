...
def split_experience_batch(experience: Experience, data_processor: Optional[BaseDataProcessor], args=None) -> List[BufferItem]:
...
def make_experience_batch(items: List[BufferItem], data_processor: Optional[BaseDataProcessor], packing_samples=False, args=None) -> Experience:
    kwargs = {}
    keys = (
        "sequences",
        "action_log_probs",
...
def remove_padding_in_sequences(items):
    for item in items:
        seq, act_log_prob, base_act_log_prob, value, ret, adv, att_mask, act_mask, action_flow_score = (
            item.sequences,
...
class NaiveReplayBuffer(ABC):
    ...
    def append(self, experience: Experience) -> None:
        ...
        items = split_experience_batch(experience, self.data_processor, self.args)
        # NOTE: No tested
        if self.drop_maxlen:
    ...
    def sample(self) -> Experience:
        ...
        experience = make_experience_batch(items, self.data_processor, self.packing_samples, self.args)
        if self.cpu_offload:
    ...
    def collate_fn(self, batch) -> Experience:
        experience = make_experience_batch(batch, self.data_processor, self.packing_samples, self.args)
        return experience
