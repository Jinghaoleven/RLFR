...
from openrlhf.models import Actor, Actor_Flow
from openrlhf.models.lmm_kits.utils import get_data_processor, get_tokenizer, collate_chat_fn
...
class ActorPPOTrainer(PPOTrainer):
    ...
    def _broadcast_to_vllm(self):
        ...
        torch.cuda.empty_cache()
        if isinstance(self.actor, Actor) or isinstance(self.actor, Actor_Flow):
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        if self.save_hf_ckpt:
            ...
            self.strategy.save_model(
                ...
                self.processor if args.mllm_training else self.tokenizer,
                save_path,
            )
            if args.save_flow:
                self.strategy.save_model(
                    self.actor.flow, 
                    output_dir=save_path+"_flow")
        # wait
        if not self.disable_ds_ckpt:
            if self.critic_train_remote:
                ray.get(ref)
...
class ActorModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        if args.flow_coef > 0:
            actor = Actor_Flow(
                ...
                pretrained_flow_model=args.pretrained_flow_model,
        ...
        if hasattr(args, "freeze_prefix") and args.freeze_prefix:
            frozen_count = 0
            total_params = 0
            for name, param in actor.model.named_parameters():
                total_params += 1
                if any(name.startswith(prefix) for prefix in args.freeze_prefix):
                    param.requires_grad = False
                    frozen_count += 1
            strategy.print(f"Froze {frozen_count}/{total_params} parameters based on prefixes: {args.freeze_prefix}")
        if args.flow_coef > 0 and args.freeze_flow:
            for name, param in actor.flow.model.named_parameters():
                param.requires_grad = False
            strategy.print(f"Froze all flow parameters")
        ...
        # # configure flow optimizer
        flow_optim = None
        if args.flow_coef > 0 and not args.freeze_flow:
            flow_optim = strategy.create_optimizer(
                actor.flow, lr=args.flow_learning_rate, betas=args.adam_betas, weight_decay=args.l2
            )
        ...
        flow_scheduler = None
        if flow_optim:
            flow_scheduler = get_scheduler(
                "cosine_with_min_lr",
                flow_optim,
                num_warmup_steps=math.ceil(max_steps * args.lr_warmup_ratio),
                num_training_steps=max_steps,
                scheduler_specific_kwargs={"min_lr": args.flow_learning_rate * 0.1},
            )
        ...
        self.flow_optim = None
        self.flow_scheduler = None
        if args.flow_coef > 0:
            self.actor.flow, self.flow_optim, self.flow_scheduler = strategy.prepare(
            (actor.flow, flow_optim, flow_scheduler),
            is_rlhf=True,
        )
    ...
    def save_model(self):
        ...
        self.strategy.save_model(
            self.ema_model if args.enable_ema else self.actor,
            self.data_processor.processor if args.mllm_training else self.tokenizer,
            args.save_path,
        )
