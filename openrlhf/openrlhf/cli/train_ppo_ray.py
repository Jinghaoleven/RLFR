...
if __name__ == "__main__":
    ...
    parser.add_argument("--baseline_mode", type=str, default=None, help="baseline mode, entropy")
    parser.add_argument("--mllm_training", action="store_true", default=False)
    parser.add_argument("--llm_training", action="store_true", default=False)
    ...
    # Flow
    parser.add_argument("--flow_config", type=str, default=None, help="Config path to flow")
    parser.add_argument("--pretrained_flow_model", type=str, default=None, help="Pretrained flow model name or path")
    parser.add_argument("--freeze_flow", action="store_true", default=False, help="whether to freeze flow in policy optimization")
    parser.add_argument("--save_flow", action="store_true", default=False, help="whether to save flow model")
    parser.add_argument("--freezing_actor_steps_for_flow", type=int, default=-1, help="Used for Flow initialization with freezed policy")
    parser.add_argument("--flow_learning_rate", type=float, default=1e-6)
    parser.add_argument("--rewarding_timesteps", type=str, default="0.8", help="Timesteps used for flow rewarding, can use like 0.5+0.8 to enable timestep collection")
    parser.add_argument("--flow_coef", type=float, default=0, help="Coefficent of flow reward")
    parser.add_argument("--flow_score_threshold", type=float, default=-1, help="Threshold to discard noisy fluctuation of flow score")
    parser.add_argument("--flow_score_topk", type=int, default=-1, help="Flow reward for topk tokens")
    parser.add_argument("--flow_gamma", type=float, default=1, help="Adopt gamma transformation for flow reward")
    parser.add_argument("--flow_reward_norm", type=str, default='minmax', help="Type for flow reward normalization")
    parser.add_argument("--flow_score_bil", action="store_true", default=False, help="whether the flow score is bilateral")
    parser.add_argument("--experience_sampling_metrics", type=str, default="batch_correct", help="metrics in experience sampling for flow optimization, can use + to enable multiple metrics")
    parser.add_argument("--metrics_entropy_quantile", type=float, default=None, help="Used in experience sampling with entropy metric, define entropy quantile")
    parser.add_argument("--soft_flow", type=float, default=0, help="soft weight for rejected experience")
    ...
    parser.add_argument("--media_key", type=str, default=None, help="JSON dataset key")
    parser.add_argument("--media_dir", type=str, default=None, help="JSON dataset media dir")
    ...
    if args.flow_reward_norm == "minmax":
        args.flow_score_bil = False
    elif args.flow_reward_norm == "minmax-bil" or args.flow_reward_norm == "mean":
        args.flow_score_bil = True
    else:
