...
def collate_chat_fn(examples):

    prompt = [example[0] for example in examples]
    labels = [example[1] for example in examples]
    output = (prompt,labels)

    return output


def load_config(config_file):
    # kwargs = {"path_or_repo_id": config_path, "cache_dir": config_path}
    # config_file = cached_file(filename="matching_config.json",**kwargs)
